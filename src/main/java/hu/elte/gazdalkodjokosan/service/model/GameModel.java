...
import hu.elte.gazdalkodjokosan.events.*;
...
public class GameModel implements CardListener {
    ...
    public void cheatStep(int fieldNumber) {
        if (gameOver || currentPlayer.equals(lastStepped)) {
            return;
        }
        ...
        int step;
        if (fieldNumber <= currentPosition) {
            step = fieldNumber + table.size() - currentPosition;
        } else {
            step = fieldNumber - currentPosition;
        }
        stepForward(step);
        System.out.println("Game cheat stepped. Current player at field: " + currentPlayer.getPosition());
    }
